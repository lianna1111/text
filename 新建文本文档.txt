...
void lemon(int m,int n)
{
	...
	if(b[m][n]==0 && x[m][n]==0)//向上走 
	{
		...
		lemon(m+1,n);//向上走 
		//运行到这里时，说明前面所走的路都不通或者已经走到终点退回来，这时候就将所走的方位标记重置，并且走过的步数减1 
		x[m][n]=0;//将这次方位标记 
		num--;
	}
	if(b[m][n]==0 && x[m][n]==0)//向下走 
	{
		...
		x[m][n]=1;//将走过的方位标记，防止下一次移动重复
		lemon(m-1,n);//向上走 
		//运行到这里，说明前面所走的路都不通或者已经走到终点退回来，这时将所走的方位标记重置，并且走过的步数减1 
		x[m][n]=0;//标记方位
		num--;
	}
	if(b[m][n]==0 && x[m][n]==0)//向右走 
	{
		...
		x[m][n]=1;//将走过的方位标记，防止下一次移动重复
		lemon(m,n+1);//向上走 
		//运行到这里，说明前面所走的路都不通或者已经走到终点退回来，这时候就将所走的方位标记重置，并且走过的步数减1 
		x[m][n]=0;//将这次方位标记 
		num--;
	}
